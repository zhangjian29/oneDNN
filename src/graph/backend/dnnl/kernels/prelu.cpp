...
template <bool quantized>
void prelu_fwd_t<quantized>::prepare_args_set(const execution_args_set_t *res,
...
void prelu_bwd_t::prepare_args_set(const execution_args_set_t *res,
        const std::vector<tensor_t> &inputs,
        const std::vector<tensor_t> &outputs, const scratchpad_t &scratchpad) {
    // update the data of partition in/outputs args
    for (const auto &mem_idx : res->get_mems_use_external_inputs()) {
...
template struct prelu_fwd_t<false>;
template struct prelu_fwd_t<true>;
} // namespace dnnl_impl
} // namespace graph
} // namespace impl
} // namespace dnnl
